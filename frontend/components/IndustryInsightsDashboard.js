...
import './IndustryInsightsDashboard.css';
...
const IndustryInsightsDashboard = () => {
      ...
      return (
        <div className="custom-tooltip">
          ...
          {selectedInsightType === 'skills' && (
            <>
              <p className="tooltip-value">{`Demand: ${payload[0].value}`}</p>
              <p className="tooltip-growth">{`Growth: ${insightsData[selectedIndustry].skills.find(item => item.name === label).growth}`}</p>
            </>
          )}
          {selectedInsightType === 'salaries' && (
            <>
              <p className="tooltip-value">{`Salary: ${formatCurrency(payload[0].value)}`}</p>
              <p className="tooltip-growth">{`Growth: ${insightsData[selectedIndustry].salaries.find(item => item.name === label).growth}`}</p>
            </>
          )}
          {selectedInsightType === 'growth' && (
            <>
              <p className="tooltip-value">{`Growth Rate: ${payload[0].value}%`}</p>
              <p className="tooltip-current">{`Current Demand: ${insightsData[selectedIndustry].growth.find(item => item.name === label).current}`}</p>
              <p className="tooltip-future">{`Future Projection: ${insightsData[selectedIndustry].growth.find(item => item.name === label).future}`}</p>
            </>
          )}
        </div>
      );
  ...
  return (
    <div className="industry-insights-dashboard">
      ...
      <div className="insights-controls">
        <div className="industry-selector">
          ...
          <div className="industry-buttons">
            {industries.map((industry) => (
              <button
                key={industry.id}
                className={`industry-button ${selectedIndustry === industry.id ? 'selected' : ''}`}
        ...
        <div className="insight-type-selector">
          ...
          <div className="insight-type-buttons">
            <button
              className={`insight-type-button ${selectedInsightType === 'skills' ? 'selected' : ''}`}
            ...
            <button
              className={`insight-type-button ${selectedInsightType === 'salaries' ? 'selected' : ''}`}
            ...
            <button
              className={`insight-type-button ${selectedInsightType === 'growth' ? 'selected' : ''}`}
      ...
      <div className="insights-visualization">
        <h3>
          ...
          {selectedInsightType === 'skills' && (
            <>
              <div className="summary-card">
                ...
                <p className="insight-value">{insightsData[selectedIndustry].skills[0].name}</p>
                <p className="insight-detail">With a demand score of {insightsData[selectedIndustry].skills[0].value} and growth of {insightsData[selectedIndustry].skills[0].growth}</p>
              </div>
              <div className="summary-card">
                ...
                <p className="insight-detail">With a growth rate of {
                  insightsData[selectedIndustry].skills.reduce((prev, current) => {
                    const prevGrowth = parseInt(prev.growth.replace('%', '').replace('+', ''));
                    const currentGrowth = parseInt(current.growth.replace('%', '').replace('+', ''));
                    return prevGrowth > currentGrowth ? prev : current;
              ...
              <div className="summary-card">
                ...
                <p className="insight-detail">Focus on developing the top 3 skills to maximize your market value in this industry.</p>
              </div>
            </>
          )}
          ...
          {selectedInsightType === 'salaries' && (
            <>
              <div className="summary-card">
                ...
                <p className="insight-value">{insightsData[selectedIndustry].salaries[0].name}</p>
                <p className="insight-detail">With an average salary of {formatCurrency(insightsData[selectedIndustry].salaries[0].value)}</p>
              </div>
              <div className="summary-card">
          ...
          {selectedInsightType === 'growth' && (
            <>
              <div className="summary-card">
                ...
                <p className="insight-value">{insightsData[selectedIndustry].growth[0].name}</p>
                <p className="insight-detail">With projected growth of {insightsData[selectedIndustry].growth[0].value}%</p>
              </div>
              <div className="summary-card">
                ...
                <p className="insight-detail">Have a "Very High" future demand outlook</p>
              </div>
              <div className="summary-card">
